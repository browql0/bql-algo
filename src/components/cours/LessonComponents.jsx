...
const renderMockSyntax = (code) => {
  ...
  return code.split('\n').map((line, li) => {
    ...
    while (i < line.length) {
      ...
      if (/[();:,\[\].]/.test(line[i])) { tokens.push({ type: 'punct', v: line[i] }); i++; continue; }
...
export const LoopDiagram = ({ type = 'pour', initLabel = 'Init', condLabel = 'Condition', bodyLabel = 'Corps', updateLabel = 'Update' }) => (
    ...
    <div style={{ position: 'fixed', bottom: '1.5rem', right: '1.5rem', zIndex: 200 }}>
      {open && (
        <div style={{
          ...
          right: isMob ? 0 : 0,
          left: isMob ? 0 : 'auto',
          top: isMob ? 'auto' : undefined,
          marginBottom: isMob ? 0 : '3.8rem',
